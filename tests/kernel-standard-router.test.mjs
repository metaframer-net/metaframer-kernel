...
test("valid POST /customers dispatches exactly once and returns the same response object", () => {
  let callCount = 0;
  let receivedMessage;
  ...
  const result = router.handle(message);
...
test("malformed message, unknown path and unsupported method all short-circuit without calling any handler", () => {
  let called = false;
  ...
  assert.equal(router.handle({ method: "POST", path: "/unknown" }).status, 404);
  assert.equal(router.handle({ method: "DELETE", path: "/customers" }).status, 405);
  assert.equal(called, false);
});

test("handler returning a non-object throws TypeError", () => {
  for (const badReturn of ["not-an-object", null, []]) {
    ...
    assert.throws(() => router.handle({ method: "POST", path: "/customers" }), TypeError);
  }
});
