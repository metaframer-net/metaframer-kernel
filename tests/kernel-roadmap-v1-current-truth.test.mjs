...
test('global readiness truth is all false and current truth names real existing and missing pieces', () => {
  ...
  for (const term of ['generic contract-to-SDK', 'app-core', 'outbox relay', 'production proof']) {
...
test('roadmap.progress carries the exact 6/25 completed truth with P05 closed and P07 active', () => {
  const doc = loadRoadmap();
  ...
  assert.deepEqual([...progress.completedPackages].sort(), ['P01', 'P02', 'P03', 'P04', 'P05', 'P06']);
  ...
  assert.equal(progress.activePackage, 'P07');
  assert.equal(progress.asOfKernelMain, 'd11b17ccfb4d53d6a0b138bf8ff07bcdefebb3e0');
  assert.equal(progress.statusLine, '6/25 tamamlandı, P07/25 aktif');
...
test('currentTruth reflects P05 (UoW, CommitReceipt, WriteEnvelope, commit-boundary composition) as implemented and P07 as the explicit next-missing piece, with all readiness flags false', () => {
  const doc = loadRoadmap();
  const t = doc.currentTruth;

  const implemented = t.implementedPieces.join(' | ');
  assert.match(implemented, /UoW|UnitOfWork/);
  assert.match(implemented, /CommitReceipt/);
  assert.match(implemented, /write envelope/i);
  assert.match(implemented, /commit.boundary composition/i);
  assert.match(implemented, /P05/);

  const missing = t.notImplementedPieces.join(' | ');
  assert.doesNotMatch(missing, /UoW/);
  assert.doesNotMatch(missing, /CommitReceipt/);
  assert.doesNotMatch(missing, /write envelope/i);
  assert.doesNotMatch(missing, /\(P05\)/);
  assert.match(missing, /generic contract-to-SDK/);
  assert.match(missing, /P07/);
  ...
  assert.match(ownerText, /6\/25/);
  assert.match(ownerText, /P07/);
  assert.doesNotMatch(ownerText, /\bis runnable\b/i);
});

test('ROADMAP.md, README.md and CHANGELOG.md project P05 closed / P07 active, and src/delivery/create-customer-asgi-composition.mjs no longer misdescribes the commit boundary as a PostgresCommitAdapter', () => {
  const roadmap = readText('ROADMAP.md');
  assert.match(roadmap, /6\/25 tamamlandı, P07\/25 aktif/);
  assert.match(roadmap, /P05/);
  assert.match(roadmap, /P07/);

  const readme = readText('README.md');
  assert.match(readme, /6\/25 tamamlandı, P07\/25 aktif/);

  const changelog = readText('CHANGELOG.md');
  const unreleasedIdx = changelog.indexOf('## [Unreleased]');
  const unreleased = changelog.slice(unreleasedIdx);
  assert.match(unreleased, /P05/);
